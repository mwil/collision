...
def do_gen(content, decision, wide):
    ...
    SER_S = np.zeros((tau_range.shape[0], Au_range.shape[0]), dtype=np.float)
    SER_U = np.zeros((tau_range.shape[0], Au_range.shape[0]), dtype=np.float)
    ...
    for tau_idx in xrange(tau_range.shape[0]):
        ...
        for Au_idx in xrange(Au_range.shape[0]):
            ...
            print "ser_contour_AsAu: tau=%.4f, Au=%.4f, step=%i/%i, runtime: %.2f secs" %\
                (tau, Au, Au_idx, Au_range.shape[0], time.time() - start_time)
            start_time = time.time()
...
if __name__ == '__main__':
    input_params = (('content', ('same', 'unif'), 'Data content'),
                    ...
                    ('wide', ('', '_wide'), 'Time offset interval'))
