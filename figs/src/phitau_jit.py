...
def detect_i(alpha, beta, phi_range, tau, As, Au):
    ...
    # numba does not support tuple unpacking yet
    beta_pack = concatenate((beta[0], zeros(max(1, 2*abs(k_tau))))), concatenate((beta[1], zeros(max(1, 2*abs(k_tau_n)))))
    beta_i = beta_pack[0] 
    beta_q = beta_pack[1]
    ...
    PA_TMP = meshgrid(phi_range, alpha[0])
    PHI_C = PA_TMP[0]
    ALPHA = PA_TMP[1] 
    ARG12 = meshgrid(phi_range, (arg1 - arg2))[1]
    ARG34 = meshgrid(phi_range, (arg3 + arg4))[1]
...
def detect_q(alpha, beta, phi_range, tau, As, Au):
    ...
    beta_pack = concatenate((beta[0], zeros(max(1, 2*abs(k_tau_p))))), concatenate((beta[1], zeros(max(1, 2 * abs(k_tau)))))
    beta_i = beta_pack[0]
    beta_q =  beta_pack[1]
    ...
    PA_TMP = meshgrid(phi_range, alpha[1])
    PHI_C = PA_TMP[0]
    ALPHA = PA_TMP[1]
    ARG12 = meshgrid(phi_range, (arg1 - arg2))[1]
    ARG34 = meshgrid(phi_range, (arg3 + arg4))[1]
...
def map_chips(vsyms, asyms):
    ...
    alpha_i = array([], dtype=int32)
    alpha_q = array([], dtype=int32)
    beta_i  = array([], dtype=int32)
    beta_q  = array([], dtype=int32)
...
def channel(alpha, beta, phi_range, tau, As, Am):
    RECV_CHIPS_I = detect_i(alpha, beta, phi_range, tau, As, Am)
    RECV_CHIPS_Q = detect_q(alpha, beta, phi_range, tau, As, Am)
...
def detect_syms_corr(recv_chips):
    ...
    while len(recv_chips) > 0:
        ...
        for sym in chips.iterkeys():
            curr_corr = abs(correlate(chips[sym], curr_chips))[0]
...
def detect_syms_corrcoef(recv_chips, **args):
    ...
    while len(recv_chips):
        ...
        best_syms = where(corr_matrix >= max(corr_matrix))[0]
